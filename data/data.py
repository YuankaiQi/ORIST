...
def get_gather_index(txt_lens, num_bbs, batch_size, max_len, max_bb_len, out_size):
    ...
    for i, (tl, nbb) in enumerate(zip(txt_lens, num_bbs)):
        ...
        if max_bb_len!=-1: # -1 denotes not use orientation
            gather_index.data[i, tl+nbb] = max_len+max_bb_len # angle emb index, not effect if not considered in attn_mask
