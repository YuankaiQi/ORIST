...
from r2r_utils import (read_vocab,write_vocab,build_vocab,Tokenizer,VLNBertTokenizer,timeSince,
                       read_img_features,create_links,reduce_tensor)

from env import R2RBatch
from agent_r2r_multiloss import BertAgent
from eval import MultiEvaluationNDH
from ndh_param import args
import warnings
warnings.filterwarnings("ignore")
from datetime import datetime
from tensorboardX import SummaryWriter
metrics = ['length','nav_error','oracle success_rate','success_rate','spl',
           'oracle path_success_rate','goal_progress', 'angle_acc', 'next_region_acc',
           'target_region_acc', 'candi_region_acc']
...
def valid(train_env, tok, val_envs={}, aug_env=None):
    # writer = SummaryWriter(logdir=log_dir)

    listner = BertAgent(train_env, "", tok, args.maxAction,args)

    best_val = {'val_seen': {"goal_progress": 0., "state": "", 'update': False},
                'val_unseen': {"goal_progress": 0., "state": "", 'update': False}}
    ...
    for env_name, (env, evaluator) in val_envs.items():
        ...
        if args.local_rank == 0:
            ...
            instr_ids = []
            for item in result:
                if item['inst_idx'] not in instr_ids:
        ...
        if env_name != 'test':
            ...
            if args.local_rank == 0:
                ...
                for i, metric in enumerate(metrics):
                    ...
                    if metric in ['goal_progress']:
                        if env_name in best_val:
                            if val > best_val[env_name]['goal_progress']:
    ...
    if args.local_rank == 0:
        ...
        LOGGER.info("*****  BEST RESULT TILL NOW--%s" % args.path_type)
        for env_name in best_val:
...
def train(train_env, tok, val_envs={}, aug_env=None):
    ...
    #     TB_LOGGER.create(args.log_dir)
    #     # pbar = tqdm(total=args.num_train_steps)
    #     add_log_to_file(join(args.log_dir, 'log_start_from_%s.txt'%listner.start_epoch))
    # else:
    #     LOGGER.disabled = True
        ...
        if aug_env is None:  # The default training process
            ...
            for step, batch_ids in enumerate(train_loader):
                ...
                for i, param_group in enumerate(listner.optimizer.param_groups):
                    if i < len(listner.optimizer.param_groups) - 2:
                ...
                # update params
                if args.grad_norm != -1:
                    grad_norm = torch.nn.utils.clip_grad_norm_(listner.model.parameters(),args.grad_norm)
        ...
        for env_name, (env, evaluator) in val_envs.items():
            ...
            if args.local_rank == 0:
                ...
                for i, metric in enumerate(metrics):
                    ...
                    loss_str[env_name] += '%s: %.3f,' % (metric, val)
                    if metric in ['goal_progress']:
                        TB_LOGGER.add_scalar("goal_progress/%s" % env_name, val, epochId)
                        if env_name in best_val:
                            if val > best_val[env_name]['goal_progress']:
                                best_val[env_name]['goal_progress'] = val
                                best_val[env_name]['update'] = True
                    elif metric in ['spl', 'success_rate', 'trajectory_lengths', 'angle_acc', 'next_region_acc', 'target_region_acc', 'candi_region_acc']:
...
def train_val():
    ...
    LOGGER.info('use_history_state = %s' % str(not args.no_history_state))
    ...
    if feedback_method=='sample' and args.ml_weight != 0:
        train_env = R2RBatch(feat_dict, batch_size=int(args.train_batch_size/2), splits=['train'], tokenizer=tok, seed=args.seed,
    ...
    val_envs = OrderedDict(
        ((split,
          (R2RBatch(feat_dict, batch_size=args.val_batch_size, splits=[split], tokenizer=tok,seed=args.seed, bert_tokenizer=bert_tok,
           ...
           MultiEvaluationNDH([split], featurized_scans, tok, args=args))
          )
...
if __name__ == "__main__":
    ...
    LOGGER.info("device: {} n_gpu: {}/{}, rank: {}, "
                "16-bits training: {}".format(device, args.n_gpu, n_gpu, args.local_rank, args.fp16))
