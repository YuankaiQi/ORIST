...
class VLNOrienEmbeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.orien_embeddings = nn.Linear(config.angleFeatSize, config.hidden_size)
        self.orien_norm = FusedLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, orien):
        emb = self.orien_embeddings(orien)
        norm = self.orien_norm(emb)
        norm_drop = self.dropout(norm)
...
class UniterModel(UniterPreTrainedModel):
    ...
    def __init__(self, config, img_dim):
        ...
        self.orien_embeddings = VLNOrienEmbeddings(config)
    ...
    def _compute_img_txt_embeddings(self, input_ids, position_ids,
                                    ...
                                    instr_state=None, candi_leng=None,
                                    orien_feat=None, step=0):
        txt_emb = self._compute_txt_embeddings(
            input_ids, position_ids, txt_type_ids)

        if self.use_lstm:
            if step > 0:
                for i in range(len(candi_leng)):
                    txt_emb[sum(candi_leng[:i]):sum(candi_leng[:i]) + candi_leng[i], 0, :] = instr_state[i,:]
        else:
            for i, istate in enumerate(instr_state):
                if istate is not None:
                    txt_emb[sum(candi_leng[:i]):sum(candi_leng[:i]) + candi_leng[i], 0, :] = istate
        ...
        if orien_feat is not None:
            orien_emb = self.orien_embeddings(orien_feat).unsqueeze(1)
            embedding_output = torch.gather(torch.cat([txt_emb, img_emb, orien_emb], dim=1),
    ...
    def forward(self, input_ids, position_ids,
                ...
                img_masks=None,
                output_all_encoded_layers=True,
                txt_type_ids=None, img_type_ids=None,candi_leng=None,
                orien_feat=None, instr_state=None, step=0):
        ...
        if input_ids is None:
            ...
            embedding_output = self._compute_img_txt_embeddings(
                ...
                img_feat, img_pos_feat,
                gather_index, img_masks,
                txt_type_ids, img_type_ids,
                instr_state, candi_leng, orien_feat, step)
                # what img_masks and img_type_ids are used for?
